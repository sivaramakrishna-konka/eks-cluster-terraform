...
resource "aws_iam_role" "external_dns_pod_role" {
  name = "${var.environment}-${var.project}-eks-external-dns-pod-role"
...
resource "aws_iam_role_policy" "external-dns-pod-role-policy" {
  name = "${var.environment}-${var.project}-external-dns-pod-role-policy"
  depends_on = [aws_iam_role.aws_alb_controller_role]
